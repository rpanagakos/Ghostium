...
object TabsBinding {
    ...
    fun TextView.convertToHtmlString(textDescription: String?) {
       ...
       */
        ...
        if (htmlBody.length > 150) {
            ...
            val strBuilder = this.convertToSpannableStringBuilder(displayedText, null)
            val onClickCallback: ((String) -> Unit) = { url -> this.text = htmlBody }
            this.setTextViewLinkHtml(strBuilder, onClickCallback)
        } else
            this.text = htmlBody
    }
    ...
    fun TextView.displayHtmlString(text: String?) {
        ...
        val strBuilder = this.convertToSpannableStringBuilder(text, imageGetter)
        this.setTextViewLinkHtml(strBuilder, null)
    }
